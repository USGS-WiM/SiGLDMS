    ...
    function ($scope, $rootScope, $cookies, $location, $http, $uibModal, $stateParams, $filter, ORGANIZATION_SYSTEM, PROJECT, DATA_MANAGER, ROLE, allRoles, thisDM, dmProjects) {
        if ($cookies.get('siGLCreds') === undefined || $cookies.get('siGLCreds') === "") {
            ...
            if (thisDM !== undefined) {
                ...
                $scope.dmOrg = $scope.$parent.allORG_RES.filter(function (o) { return o.organization_system_id == $scope.DM.organization_system_id; })[0];
                ...
                $scope.UpdateMyOrg = function () {
                    ...
                    var chosenOrg = $scope.allORG_RES.filter(function (orgS) { return orgS.sec_id == secID && orgS.div_id == divID && orgS.org_id == orgID; })[0];
                    if (chosenOrg !== undefined) {
                        ...
                        ORGANIZATION_SYSTEM.save(newORG_SYS, function success(response) {
                            ...
                            orgResource.org_id = orgID; orgResource.OrganizationName = $scope.allOrgs.filter(function (o) { return o.organization_id == orgID; })[0].organization_name;
                            if (divID > 0) {
                                orgResource.div_id = divID; orgResource.DivisionName = $scope.$parent.allDIVs.filter(function (d) { return d.division_id == divID; })[0].division_name;
                            }
                            if (secID > 0) {
                                orgResource.sec_id = secID; orgResource.SectionName = $scope.$parent.allSECs.filter(function (s) { return s.section_id == secID; })[0].section_name;
                            }
                ...
                $scope.save = function (valid) {
                    if (valid) {
                        ...
                        if ($scope.selectedOrgID !== "") {
                            ...
                            var existingOrgRes = $scope.allORG_RES.filter(function (or) { return or.org_id == $scope.selectedOrgID && or.div_id == divID && or.sec_id == secID; })[0];
                ...
                $scope.postNewDM = function () {
                    DATA_MANAGER.addDataManager({ pass: $scope.pass.confirmP }, $scope.DM, function success(response) {
                        ...
                        var nm = response;
                        var org = $scope.allORG_RES.filter(function (or) { return or.organization_system_id == nm.organization_system_id; })[0];

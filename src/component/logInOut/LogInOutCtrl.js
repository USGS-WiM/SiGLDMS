(function () {
  ...
  //login
  LogInOutController.controller("loginCtrl", [
    "$scope",
    "$window",
    "$state",
    "$http",
    ...
    function (
      ...
      $scope.forgotPassword = function () {
        ...
        var modalInstance = $uibModal.open({
          template:
            '<div class="modal-header"><h3 class="modal-title">Forgot your password?</h3></div>' +
      ...
      $scope.submit = function () {
        ...
        LOGIN.login(
            ...
            if (user !== undefined) {
              ...
              if ($rootScope.returnToState !== undefined) {
                $state.go($rootScope.returnToState, {
                  id: $rootScope.returnToStateParams,
                });
              } else {
          ...
          function error(errorResponse) {
            ...
            var modalInstance = $uibModal.open({
              template:
                '<div class="modal-header"><h3 class="modal-title">Error</h3></div>' +
  ...
  //logOut
  LogInOutController.controller("logoutCtrl", [
    "$scope",
    "$rootScope",
    "$cookies",
    "$location",
    function ($scope, $rootScope, $cookies, $location) {
      $scope.logout = function () {
        ...
        $cookies.remove("pl_reverse");
        $cookies.remove("siteListSortOrder");
        $cookies.remove("sl_reverse");
        $cookies.remove("DMListSortOrder");
        $cookies.remove("dml_reverse");
        $cookies.remove("DMprojectsSortOrder");
        $cookies.remove("dmpl_reverse");
